...
suspending(#event_info{event = ?EVENT_FINISH,
    ...
    InProgressPids_1 = lists:delete(FinishedId, InProgressPids),
    ChildPids_1 = orddict:store(Pid, false, ChildPids),

    NextState = ?ST_SUSPENDING,
    {next_state, NextState, State#state{status = NextState,
                                        ongoing_targets = InProgressPids_1,
                                        child_pids      = ChildPids_1}};
...
suspending(#event_info{event = ?EVENT_FINISH,
    ...
    ChildPids_1 = orddict:erase(Pid, ChildPids),
...
-spec(loop(CallbackMod, Params) ->
             ok | {error, any()}
                 when CallbackMod::module()|undefined,
                      Params::{atom(),atom(),atom(),atom()}|undefined).
loop(CallbackMod, Params) ->
    receive
        {run, Id, IsDiagnose, IsRecovering} ->
            ...
            case leo_compact_fsm_worker:run(
                ...
                ok ->
                    ...
                    loop(CallbackMod, {Id, Id_1, IsDiagnose, IsRecovering});
                {error,_Cause} ->
                    ok = finish(self(), Id)
            end;
        run ->
